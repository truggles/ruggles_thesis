...
decays to a pair of $\tau$ leptons and uses CMS $\pp$ collision data gathered in 2016. 
...
collected at center-of-mass energy 13\TeV from the LHC. Combining
...
There are many similarities between the $ggH$ and VBF targeted $\htt$ analysis
...
production mechanisms, $\PW\PH$/$\PZ\PH$. A study of the Higgs boson produced in the $ggH$ or VBF processes
is presented in Chapter~\ref{sec:htt_analysis}.
In the following pages the symbol $\ell$ refers to light leptons (electrons and muons) and $\tauh$ refers to hadronically
decaying $\tau$ leptons. Leptons refers to electrons, muons, and $\Pgt$ including their decay products:
$\tau_{e}$, $\tau_{\mu}$, $\tauh$.
...
This combination of $\Pgt\Pgt$ final states covers about 94\% of all possiblities.
...
The presence of two simultaneous leptons in the double lepton
triggers allows for lower $\pt$ thresholds on each lepton which increases
the acceptance of $\PZ\PH$ events. 
...
single lepton triggers. This helps increase the overall trigger selection
efficiency.
...
is the result of optimizing the selection for signal significance. 
...
The subleading $\tauh$ must pass $\pt > 20\GeV$. 
...
products of the $\PH$ boson. This splitting helps separate the $\PZ+\text{jets}$
background from the $\PZ\PH$ signal which is more dominant in the
High-$L_{\text{T}}^{\textrm{Higgs}}$ region.
The $L_{\text{T}}^{\textrm{Higgs}}$ regions are defined based on the $\htt$ final 
...
efficiency \texttt{Medium Tau MVA}. In the $\PW\PH$ final states the 
hadronic final states use a combination of two working points; the 
...
The discrimanants have a range of thresholds with working points being
...
\hline
  Channel            &      Additional $\tauh$ Criteria  \\
\hline
...
\hline
  Channel           &        Additional Lepton Criteria  \\
\hline
...
contribution of these processes with jets misidentified as leptons in the signal region. 
...
the method is essentially the same, except that the lepton susceptible to being misidentified,
...
leptons in signal region events. The misidentification rate is measured in four lepton final states which
...
to being misidentified jets are the $\PH$ boson leptons.
In the $\PZ\PH$ final states, data events which pass the full
signal region selection, except either or both of the $\PH$ boson leptons 
...
events with both $\PH$ boson leptons failing have their weight
...
data in a signal free region with same charge $\PH$ boson leptons. The
contribution of irreducible backgrounds in the same charge region used to 
derive the shape template is less than 1\%. This means that the data derived template is from
...
statistical uncertainties for the shape template. Kolmogorov-Smirnov tests have been performed to validate
shape compatibility between this relaxed same charge selection and the signal region
reducible background distributions.
The events estimated from these misidentification methods are labled
...
The following Chapter~\ref{sec:cmb_results} discusses results for the combination of the
...
\begin{figure}[h!]
 ...
 \caption{The postfit $\mtt$ distributions used to extract the signal shown
  ...
  signal strength value of $2.5 \times \text{SM}$.
 }
...
visible mass of the $\tauh$ leptons in the $\ell\tauh\tauh$ final states, 
