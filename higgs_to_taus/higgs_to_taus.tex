\chapter{Higgs $\to \Pgt\Pgt$: Gluon Fusion and Vector Boson Fusion}
...
on the number of reconstructed loosely identified electrons and muons, see Section~\ref{sec:htt_obj_sel}. 
This ensures that no data or simulated event is double counted in two channels.
...
Selected events are required to to have passed a high level trigger consistent with their categorized final
state channel. For leptons at the HLT, there are multiple isolation and identification working points.
Depending on the physics needs of an HLT path, a working point will be selected that leads to the highest
efficiency possible while maintaining a rate which fits into the allocated rate budget. Descriptions of the working points
can be found in References~\cite{Khachatryan:2016bia, CMS-PAS-TAU-16-002, Khachatryan:2015hwa}.

The HLT paths used in this analysis, the lepton $\pt$ thresholds, and $\abs\eta$ 
requirements are detailed in table~\ref{tab:htt_hlt_triggers}. 
For the $\Pgm\tauh$ channel, a combination of single muon and muon-$\tauh$ cross triggers
are used. The cross trigger allows for a lower $\pt$ threshold on the muon increasing
signal acceptance.
In contrast, due to the HLT menu available in 2016, the $\Pe\tauh$ cross triggers
do not bring a substantial increase in acceptance and were not used in this analysis.
For the $\Pe\tauh$ channel, events are selected using a trigger requiring a 
single tightly isolated electron. For the $\Pe\Pgm$ channel, events are selected with electron-muon cross
triggers. For the $\tauh\tauh$ channel, events are selected with an HLT path requiring two loosely isolated $\tauh$.

Due to changing data gathering conditions and a changing HLT menu throughout 2016 data taking, the
2016 dataset is broken down into different time dependent eras beginning with ``B'' and sequentially
progressing to ``H.'' 
During the final eras of 2016, the instantaneous 
luminosity and the average pileup per event increased compared to the initial eras. Because of this,
some HLT paths became disabled or prescaled to keep the overall L1 Trigger and HLT rate within
the allocated budget. This study avoids using any prescaled HLT paths. 

As the instantaneous luminosity increased the HLT paths for the $\Pgm\tauh$,
$\Pe\Pgm$, and $\tauh\tauh$ channel changed to maintain the budgeted rate.
The single muon triggers introduced a tighter $\abs\eta$ restrictions.
Towards the end of 2016, the $\Pe\Pgm$ triggers introduced a $\Delta \text{z}$
requirement matching the leptons to the primary vertex (Section~\ref{sec:pv_reco}) to reduce rate.
The isolation criteria was changed for the $\tauh\tauh$ channel triggers to maintain a reasonable rate. The $\tauh$ 
isolation changed from being based on purely charged energy deposits, which are defined by
energy deposits linked to a track, to being based on a 
combination of charged and neutral energy deposits. Neutral energy deposits are not linked to a
track.

In all channels, the electrons, muons, and
$\tauh$ in each event must be matched to within $\Delta R < 0.5$ with the associated
HLT object which triggered the event.

%For the $\Pgm\tauh$ channel, events are selected using a combination
...
%single tightly isolated electron. For the $\Pe\Pgm$ channel, events are selected with electron-muon cross
%triggers requiring an isolated electron and an isolated muon. There are two 
...
%with $\pt$ over 35\GeV.
%
%For the $\Pgm\tauh$ channel this 
%leads to changes in the $\abs\eta$ requirement on the muon in the single muon triggers.
...
%Due to changing pileup conditions (Section~\ref{sec:sim_pu}) and increasing luminosity during the final 2016 eras, the
%isolation criteria was changed for the $\tauh\tauh$ channel triggers to maintain a reasonable rate. The $\tauh$ 
...
%vertex. This is a common method used to reduce rate at the HLT. 
%In all channels, the electrons, muons, and
%$\tauh$ in each event must be matched to within $\Delta R < 0.5$ with the associated
%HLT object which triggered the event.
...
%\begin{scriptsize}
\begin{tabular}{lccc}
  Channel           &         Trigger ($\pt$/$\abs\eta$) Req.              &     High Level Trigger Description  &   Eras  \\
\hline
  $\mu\tauh$       &         $\Pgm(22/2.4)$                     &  single iso muon & B-F  \\ 
                   &         $\Pgm(22/2.1)$                     &  single iso muon & C-H  \\
                   &         $\Pgm(19/2.1)\,\&\,\tauh (20/2.1)$     &  iso muon \& loose iso $\tauh$ &  All Eras  \\
\hline
  $\Pe\tauh$       &         $\Pe(25/2.1)$                     &  single tight iso electron  & All Eras \\
\hline
 $\tauh\tauh$      &         $\tauh(35/2.1)\,\&\,\tauh (35/2.1)$   &  two iso $\tauh$ (charged isolation) & B-G   \\ 
                   &         $\tauh(35/2.1)\,\&\,\tauh (35/2.1)$   &  two iso $\tauh$ (combined isolation) & H  \\
\hline
  $\Pe\Pgm$        &         $\Pe(12/2.5)\,\&\,\Pgm(23/2.4)$       &  loose iso muon \& loose iso electron & B-F \\
                   &         $\Pe(12/2.5)\,\&\,\Pgm(23/2.4)$       &  loose iso muon \& loose iso electron \& DZ req. & G-H  \\
                   &         $\Pe(23/2.5)\,\&\,\Pgm(8/2.4)$        &  loose iso muon \& loose iso electron & B-F  \\
                   &         $\Pe(23/2.5)\,\&\,\Pgm(8/2.4)$        &  loose iso muon \& loose iso electron \& DZ req. & G-H  \\
\hline
\end{tabular}
\end{footnotesize}
%\end{scriptsize}
\caption{For each channel the HLT $\pt$ threshold and $\abs\eta$ range is listed along 
with a description of the HLT path where ``iso'' stands for ``isolated.'' Changes in the available triggers with respect to
the data collection era are noted.
...
Additionally, selected objects are required to meet the same $\abs\eta$ restrictions
as are imposed by the HLT paths.
...
require the Particle Flow Medium ID, Section~\ref{sec:pf_muons}. The selection of $\tauh$ relies on MVA-based working
points. The $\tauh$ MVA-based working points combine both object identification and object
isolation together into a single set of working points, Section~\ref{sec:obj_reco_tau}. Table~\ref{tab:htt_obj_selection}
...
remove events with a loosely identified electrons and muons. The loosened 
identification is consistent in $\eta$ with Table~\ref{tab:htt_obj_selection} while
the $\pt$ threshold is lowered to $\pt > 10\GeV$.
For electrons we use: MVA 90\% WP, $I^{\Pe}<0.3$, while for muons we use: PF ID Medium, $I^{\Pgm}<0.3$.
...
if they pass they pass the loose working point of the PF Jet ID discriminator~\cite{CMS-PAS-JME-13-005,CMS-PAS-PFT-09-001}.
...
from a b-quark are considered if they pass the combined secondary vertex (CSV) Medium working point 
...
and $\abs\eta<2.4$.  The tightened $\eta$ requirement ensures that b-tagged jets are located within
...
$p_\zeta - 0.85 \, p_\zeta^{\text{vis}} > -35\GeV$ in the ``0-jet'' and ``Boosted'' categories,
or $p_\zeta - 0.85 \, p_\zeta^{\text{vis}} > -10\GeV$ in the ``VBF'' category where the
categories are defined in the following section. 
$p_\zeta$ is the component of the \etvecmiss projected along the bisector 
...
\begin{itemize}
\item {0-jet}: This category targets Higgs boson events produced via gluon fusion (Section~\ref{sec:higgs_production}).
...
an excellent handle to distinguish $\PZ\to\ell\ell$ from the other background process and
...
\item {VBF}: This category targets Higgs boson events produced via the VBF process (Section~\ref{sec:higgs_production}).
...
category are produced via the VBF production mode. This proportion increases with $\mjj$ which allows
disambiguation of the gluon fusion and VBF signals in the highest $\mjj$ ranges.
...
made the determination to save a given event. The HLT triggers used in this analysis,
...
The simulated background processes 
are all scaled to their NLO cross section in Table~\ref{tab:htt_bkg_xsec}.
The cross sections of the Higgs boson production processes are listed in Section~\ref{sec:higgs_production}.
...
luminosity for each bunch crossing. The average number of pileup interactions in
the 2016 CMS data is approximately 27 interactions per bunch crossing, Figure~\ref{fig:pileup_2016}.
...
EWK $Z \rightarrow \ell\ell+ 2\text{jets}$& 3.987\\
EWK $Z \rightarrow \nu\nu + 2\text{jets}$ & 10.01\\
...
EWK $W^- +2\text{jets}$ & 20.25\\
EWK $W^+ +2\text{jets}$ & 25.62\\
$WZ \rightarrow 1\ell3\nu$ & 3.05 \\
$WZ \rightarrow 1\ell1\nu2Q$ & 10.71\\
$WZ \rightarrow 2\ell2Q$ & 5.595\\
EWK $WZ \rightarrow 3\ell\nu$& 4.708\\
$WW \rightarrow 1\ell1\nu2Q$ & 1.212\\ %49.997
$WW \rightarrow 2\ell2\nu$ & 12.178 \\
$ZZ \rightarrow 2\ell2Q$ & 3.22 \\
...
\label{tab:htt_bkg_xsec} 
\end{table}

\begin{figure*}[htbp]
\centering
     \includegraphics[width=0.65\textwidth]{higgs_to_taus/plots/pileup_pp_2016.pdf}
     \caption{
Mean number of interactions per bunch crossing for the 2016 proton-proton run at 13\TeV.
The cross section is taken to be 80 mb.
     }
     \label{fig:pileup_2016}
\end{figure*}


\section{Mass Reconstruction}
\label{sec:svfit}
%The observables measured in each $\htt$ event do not provide sufficient information to solve 
%for the $\Pgt$ pair mass analytically. 
...
the $\Pgt$ lepton energy. Leptonically decaying $\Pgt$ produce two neutrinos through their decay
while hadronically decaying $\Pgt$ produce a single neutrino, see Table~\ref{tab:tau_dms}.
The presence of the neutrinos reduces the discriminating power of $\mvis$.
The \textsc{svfit} algorithm~\cite{Bianchini:2014vza} combines the \etvecmiss 
with the four-vectors of both $\Pgt$ candidates
to calculate a more accurate estimate of the mass of the parent boson, denoted as $\mtt$. 
The \textsc{svfit} algorithm is based on a likelihood 
method which, given the input parameters, in discrete $\mtt$ slices, estimates the compatibility 
of $\mtt$ with the inputs. The $\mtt$ value for an event is taken from the $\mtt$ slice with the highest compatibility.
A more detailed description of the algorithm can be found
in Reference~\cite{Bianchini:2014vza}.

The resolution of $\mtt$ is between 15 and 20\% depending on the $\Pgt\Pgt$ final state.
Both mass variables, $\mvis$ and $\mtt$ are used in the analysis, as detailed in 
Section~\ref{sec:htt_categorization}. The $\mvis$ variable is preferred over $\mtt$ when the background 
...
A number of different background estimation techniques are employed in this
analysis. In all cases all Monte Carlo based backgrounds used in the techniques
...
yield are taken from simulation based on the samples and cross sections in Table~\ref{tab:htt_bkg_xsec}.
...
the shape of the background is from the simulated samples in Table~\ref{tab:htt_bkg_xsec}. The yield is estimated 
...
The control regions have one bin because they are used solely to constrain 
...
region, the template is instead derived from the background-subtracted opposite-sign region. 
...
evaluated using Kolmogorov--Smirnov (KS) tests~\cite{ks}. 
Specifically, this study relied
...
across different isolation ranges for i) the same-sign events, and ii) for the
...
Because of this, a relaxed isolation criteria for both $\tauh$ leptons, shown
in Figure~\ref{fig:htt_tautau_qcd_schematic1},
is used to derive the QCD multijet template for the $\tauh\tauh$ channel.
...
%\end{center}
%\caption{
%Kolmogorov-Smirnov test results comparing estimated QCD multijet shape templates to test shape compatibility
...
must fail the \texttt{Tight Tau MVA} requirement (Section~\ref{sec:obj_reco_tau}) which is the isolation selection used to 
...
\centering
     ...
     \caption{
Schematic depicting the range of $\tauh$ MVA working points, defined in Section~\ref{sec:obj_reco_tau}, 
ranging from very loose (\texttt{VLoose})
...
using 1) the same isolation requirements as the signal region with all selections are identical
except for the $\tauh$ charge configuration. And, 2) a second region with the
...
to the expected yield in the signal region. The large uncertainty on the VBF scale factor 
is due to the limited amount of QCD multijet events in the VBF same-sign region. 
}
\end{table*}
...
In all channels \ttbar is predicted from simulation, see Chapter~\ref{sec:simulation}. The normalization is adjusted using
a \ttbar-enriched control region orthogonal to the signal region. This control region is included
in the global fit. The control region is defined using events in the $\Pe\Pgm$ channel and is
made orthogonal from the signal region by inverting the $p_\zeta$ requirement to:
$p_\zeta - 0.85 \, p_\zeta^{\text{vis}} < -35\GeV$. The region is enriched in \ttbar by
requiring that events should contain at least one jet.
The yield of \ttbar in all channels and categories is adjusted by this singular \ttbar control region.
...
from simulation using the samples and cross sections listed in Table~\ref{tab:htt_bkg_xsec}.
...
to account for differences in reconstruction and identification efficiencies for objects between data
and simulation. These corrections are derived in
fully orthogonal regions from the analysis signal regions whenever possible. There are however a
number of
...
Correction factors are derived by the Tau POG in the $\Pgm\tauh$ channel using a tag-and-probe method
to the method defined in Reference~\cite{CMS-PAS-TAU-16-002}. Essentially,
...
of this correction is performed in the $\Pgm\tauh$ final state by the Tau POG~\cite{CMS-PAS-TAU-16-002}. The effect of
...
are measured with dedicated muon+$\tauh$ monitoring triggers which are triggers with the same $\tauh$
requirements as the $\tauh\tauh$ trigger while they have a muon instead of a second $\tauh$ to enable the
tag-and-probe method. The tag-and-probe
is conducted using the Single Muon dataset. The selection for the $\tauh$ trigger efficiency measurements 
requires one well identified and isolated muon per event which is required to fire a single muon trigger,
see Table~\ref{tab:htt_hlt_triggers} and Reference~\cite{CMS-PAS-TAU-16-002}.
...
presence of two well-identified muons (Table~\ref{tab:htt_obj_selection}) with $\pt > 25 \GeV$ 
and an invariant mass between 70 and 110\GeV.
...
These corrections range from 1\%--10\% with a relative uncertainty of 10\%.
...
VBF category selection criteria. These corrections range from 2\%--6\% and have an assigned uncertainty
of 100\%. The uncertainty covers the range of different corrections which can be measured based on
varying the measurement region phase space.
After this reweighting, good agreement between data in the 
...
Generator weights are applied on a per event basis. 
Samples produced with the \MGAMCNLO generator
contain both positive and negative event weights.
...
The goal of the analysis was to reach an observed significance for the $\htt$ process of 5.0$\sigma$.
...
5\% as mentioned in Section~\ref{sec:htt_tau_id_eff}.
...
The trigger efficiency uncertainty per $\tauh$ candidate results from the
uncertainties on the efficiency fit function where the fit is shown in Figure~\ref{fig:htt_tt_trig}
for the $\tauh\tauh$ channel. The uncertainties result from limited statistics in the $\Pe\tauh$ and $\Pgm\tauh$
channels as a fit function is not used for application of the efficiencies for these channels. 
The $\tauh$ trigger uncertainties amounts to 5\%. This results in a total 
trigger uncertainty of $5\% + 5\% = 10\%$ for processes estimated from simulation in the $\tauh\tauh$ decay channel. 
...
to rate uncertainties of 25\% and 12\%, respectively, per reconstructed $\tauh$ decay mode~\cite{CMS-PAS-TAU-16-002}. These fake $\tauh$
...
signal and background distributions and the expected yields~\cite{CMS-PAS-TAU-16-002}. The 1.2\% uncertainty is treated as uncorrelated 
...
$\Pgm\tauh$ signal region used in this analysis. To prevent over constraining the uncertainties from
...
When extracting the results from the maximum likelihood fit discussed in Section~\ref{sec:htt_results},
...
uncertainty is largely explained by the addition of two other decay channels
with $\tauh$ candidates ($\tauh\tauh$ and $\Pe\tauh$) in addition to to the $\Pgm\tauh$ measurement region
which significantly increase the available statistics reducing the uncertainty.
...
\label{sec:fake_taus_es}
The energy scale uncertainty for muons or electrons
misidentified as $\tauh$ candidates is 1.5 or 3\%, respectively~\cite{CMS-PAS-TAU-16-002}. 
It is uncorrelated between reconstructed $\tauh$ decay
...
affect the $\pt$ of each jet in an event. As the $\pt$ for each jet increases or decreases,
the number of jets with $\pt > 30\GeV$ is recomputed as is the $\mjj$ for each event. The adjusted number of
...
forward calorimeter subdetectors~\cite{CMS-PAS-JME-16-004}. The jet related \etvecmiss scale uncertainties arise from uncertainties
...
The $\htt$ analysis only applies a b-tagged jet selection (Section~\ref{sec:htt_obj_sel}) 
criteria in the $\Pe\Pgm$ channel, where the number
...
the uncertainties range from 0\% in the lowest $\mjj$ region to 6\% in the $500\GeV < \mjj < 800\GeV$ 
region~\cite{sm-htt-2017}.
...
scaling from the high-\MT ($\MT>80\GeV$) control regions to the low-\MT ($\MT<50\GeV$) signal regions.
The scaling uncertainty is obtained by comparing the \MT distributions 
of simulated and observed $\PZ \to \Pgm\Pgm$ events where
one of the muons is removed and the \etvecmiss is adjusted accordingly, to mimic $\PW+\text{jets}$ events. The
reconstructed invariant mass of the $\PZ$ in the rest frame is multiplied by the ratio of
the $\PW$ to the $\PZ$ boson masses before removing the muon.
These uncertainties range from 5 to 10\%~\cite{sm-htt-2017}.
...
by the linear uncertainty as a function of the $\tauh$ $\pt$ described in Section~\ref{sec:fake_taus_es}.


\subsubsection{QCD Multijet Uncertainties}
The uncertainty in the QCD multijet background yield in the $\ell\tauh$ and $\Pe\Pgm$ channels 
ranges from 10 to 20\%~\cite{sm-htt-2017}.
...
In the $\tauh\tauh$ channel, the uncertainty in the QCD multijet background yield reflects a combination of the statistical
uncertainties obtained from fitting the dedicated control regions with $\tauh$ candidates passing relaxed isolation 
criteria defined in Section~\ref{sec:qcd_bkg}, and for extrapolation uncertainties to the signal region. Limited 
...
one of the $\tauh$ must not meet the signal region \texttt{Tight Tau MVA} criteria 
(Section~\ref{sec:obj_reco_tau}). A schematic depiction
of the signal region, QCD multijet control region, and method validation regions are shown in 
Figure~\ref{fig:htt_tautau_qcd_schematic2}.
With both validation regions constructed to not overlap with the
...
(Right) Schematic of the further loosened MVA isolation (defined in Section~\ref{sec:obj_reco_tau}) 
selections used for the QCD multijet validation.
...
\texttt{Looser Isolation Region} on the left plot. 
     }
...
The ``measured'' QCD multijet yield in the \texttt{Tighter Validation Region} is calculated from
...
considered when extrapolating from the $\Pe\Pgm$ final state to the others~\cite{sm-htt-2017}. The \ttbar simulation is corrected 
...
They are largely due to uncertainties in the PDFs (Section~\ref{sec:sim_pdf}), 
variations of the QCD renormalization and factorization scales~\cite{Anastasiou:2016cez},
...
by following the prescriptions in Reference~\cite{Stewart:2011cf}. These uncertainties need to be taken into account because 
...
the calculations, are considered for signal events with $\pth>150\GeV$. The boosted Higgs boson shape
uncertainties are motivated by the treatment suggested in Reference~\cite{deFlorian:2016spz}.
...
shape templates in the VBF category.
...
\hline
 ...
 $\PW+ \text{jets}$ estimation & Normalization ($\Pe\Pgm$, $\tauh\tauh$): 4--20\% &  \multicolumn{1}{c}{\NA} \\
                               & Uncert. from CR ($\Pe\tauh$, $\Pgm\tauh$): $\simeq$5--15& \multicolumn{1}{c}{\NA} \\
...
QCD multijet estimation        & Normalization ($\Pe\Pgm$): 10--20\% & 5,20\% \\
                               & Uncert. from CR ($\Pe\tauh$, $\tauh\tauh$, $\Pgm\tauh$): $\simeq$5--15\% & \multicolumn{1}{c}{\NA} \\
